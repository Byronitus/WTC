...
mazename = 'obstacles'
turtle_check = False
maze_list = ['amazen','cool_maze_bro','dank_maize','mega_epic_maze_of_mazes','obstacles','sphesmaze','the_maze']
# import maze.mega_epic_maze_of_mazes as obstacles
...
valid_commands = ['mazerun','off', 'help', 'replay', 'forward', 'back', 'right', 'left', 'sprint']
valid_arg = ['top','bottom','left','right']
...
def valid_command(command):
    ...
    if command_name.lower() == 'replay':
        ...
        if arg1 in valid_arg or arg1 == '':
            return True
    else:
        return command_name.lower() in valid_commands and (len(arg1) == 0 or is_int(arg1)) or arg1 in valid_arg
...
def do_forward(robot_name, steps):
    ...
    bool = world.update_position(steps,position_x,position_y,current_direction_index)
    if obstacles.is_path_blocked(world.x_old,world.y_old,world.new_x,world.new_y):
        return True, 'Sorry, there is an obstacle in the way.'
    if bool:
        position_x,position_y = world.return_position()
...
def do_mazerun(robot_name,arg):
    """Handling the mazerun code"""
    global position_x,position_y,current_direction_index
    position_x,position_y = 0,0
    print('> ' + robot_name +  ' starting maze run..')
    end,i,which_edge = mazerun.set_end(arg)
    if len(obstacles.ob_list) > 100:
        mazerun.create_list_of_edges(obstacles.ob_list)
        mazerun.create_edges()
        new_list = mazerun.bfs((0,0),end,mazerun.graph_of_edges,i)
        end = world.mazerun(new_list)
    else:
        mazerun.go_to_edge(robot_name)
    command_output = '> ' + robot_name + which_edge
    position_x,position_y = end[0] , end[1]
    do_next = True
    return do_next, command_output
...
def robot_start():
    ...
    if turtle_check:
        world.draw_obstacles(obstacles.ob_list)
    robot_name = get_robot_name()
    output(robot_name, "Hello kiddo!")
    print(robot_name + ': Loaded ' + mazename + '.')
    if not turtle_check:
        world.show_obstacles(obstacles.ob_list)
